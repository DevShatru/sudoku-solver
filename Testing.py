...
## - Resolve IndexError when NLV overshoots (Done)
## - Rename NLV
## - Reasons for NLV
## - Print if human solves puzzle (Done)
## - Clear print space on button press (Done for left side)
...
def assign(values, s, d):
    ...
    if all(eliminate(values, s, d2) for d2 in other_values):
        if [s, d, 0] not in solvedCells and [s, d, 1] not in solvedCells:
...
##    display(b)
    ...
    while not(isSolved("".join(sa))):
        ...
        while i < 81:
            if sa[i] != sb[i]:
                sa[i] = sb[i]
                if [Coord(i), sb[i], 0] not in solvedCells and [Coord(i), sb[i], 1] not in solvedCells:
...
##                        print list(decodeDict(b))
                    ...
                    if a[square2Cell(solvedCells[i][0])] == " ":
                        font = pygame.font.Font(None, int(windowWidth * 0.01875))
                        mesg =  solvedCells[i][1] + " in " + solvedCells[i][0] + " by "
                        if solvedCells[i][2] == 0:
                            mesg = mesg + "Deduction."
                        else:
                            mesg = mesg + "Backtracking."
